...
def preprocess_annotations(paragraphs, data):
    paragraph_ids = set([row["id"] for row in paragraphs])
    filtered_data = [row for row in data if row["par_id"] in paragraph_ids]
...
def eval_ner(data, model_result):
    ...
    with open("../results/wikineural/results.txt", "w") as output:
    ...
    tp_file = open("../results/wikineural/tp_ner.csv", "w", encoding="utf-8")
    dict_writer = csv.DictWriter(tp_file, p_keys)
    dict_writer.writeheader()
    dict_writer.writerows(matches)
    tp_file.close()

    fp_file = open("../results/wikineural/fp_ner.csv", "w", encoding="utf-8")
    dict_writer = csv.DictWriter(fp_file, fp_keys)
    dict_writer.writeheader()
    dict_writer.writerows(fp)
    fp_file.close()

    fn_file = open("../results/wikineural/fn_ner.csv", "w", encoding="utf-8")
...
with open("../scripts_extraction/annotations.csv", "r", encoding="utf-8") as f2:
    data = csv.DictReader(f2)
    data = list(data)
f2.close()

with open("../results/wikineural/output.csv", "r", encoding="utf-8") as f3:
    model_result = csv.DictReader(f3)
    model_result = list(model_result)
f3.close()

filtered_data = preprocess_annotations(paragraphs, data)
eval_ner(filtered_data, model_result)
