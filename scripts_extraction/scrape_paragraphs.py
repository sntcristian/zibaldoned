...
initial_paragraph = "https://digitalzibaldone.net/node/p4525_6"
...
def scrape_paragraphs_recursive(paragraph_num):
    if paragraph_num != None:
        ...
        if response.status_code == 200:
            ...
            elif paragraph_num == "https://digitalzibaldone.net/node/p4525_7":
...
if len(output)>0:
    ...
    with open("../data/paragraphs_all.csv", "w", encoding="utf-8") as f:
...
if len(places)>0:
    ...
    with open("places_all.csv", "w", encoding="utf-8") as f:
...
if len(persons)>0:
    ...
    with open("people_all.csv", "w", encoding="utf-8") as f:
        dict_writer = csv.DictWriter(f, p_keys)
        dict_writer.writeheader()
...
if len(works)>0:
    w_keys = works[0].keys()
    with open("works_all.csv", "w", encoding="utf-8") as f:
